...
func (s *Server) GetThumbnail(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("thumbnail generation failed for %s: %v — serving original as fallback", photo.Path(), err)
		// Mieux vaut afficher l'image d'origine (plus lourde) qu'une vignette
		// cassée ou une erreur 500 côté client.
		http.ServeFile(w, r, photo.Path())
		return
	}
